...
```
cmd/crashcart/main.go   subcommands: serve | migrate | retention | alerts | seed | export | import
internal/
  ...
  export/       export.go (NDJSON dump) + import.go (idempotent load; D1 → Go upgrade path)
...
- Never hand-write SQL in Go outside `internal/db/queries` (exceptions: migrations,
  `db.Migrate`, and the catalog-driven generic statements in `internal/export`).
