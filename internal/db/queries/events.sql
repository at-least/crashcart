...
SELECT COALESCE((SELECT e.event_id FROM events e WHERE e.project_id = sqlc.arg(project_id)::bigint AND e.fingerprint = sqlc.arg(fingerprint)::text ORDER BY e.occurred_at DESC LIMIT 1), '')::text AS latest,
       COALESCE((SELECT e.event_id FROM events e WHERE e.project_id = sqlc.arg(project_id)::bigint AND e.fingerprint = sqlc.arg(fingerprint)::text ORDER BY e.occurred_at ASC LIMIT 1), '')::text AS oldest;
