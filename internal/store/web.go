...
func LatestIssueEvent(ctx context.Context, db DB, projectID int64, fingerprint *sentry.ID, from, to time.Time) (Event, error) {
	return scanOne[Event](db.Query(ctx, "SELECT "+eventColumns+" FROM events WHERE project_id = $1 AND fingerprint = $2 AND occurred_at >= $3::timestamptz AND occurred_at < $4::timestamptz ORDER BY occurred_at DESC LIMIT 1",
