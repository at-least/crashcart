...
events*              (per project; events whose payload is in Postgres by occurred_at, event_id, then events packed in the blob store pack by pack, offset order — so an export reads each pack once)
